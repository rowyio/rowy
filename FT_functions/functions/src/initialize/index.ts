...
import { db, auth } from '../firebaseConfig'
 const initializedDoc =( columns: { fieldName: string, type:string, value: any, script: any }[]) => async(snapshot:functions.firestore.DocumentSnapshot) =>
    columns.reduce(
        async (acc, column) => {
            ...
            if (column.type === "static") {
                ...
                return { ...(await acc), [column.fieldName]: await column.script({
                    row: snapshot.data(),
                    ref:snapshot.ref,
                    db,
                    auth,
